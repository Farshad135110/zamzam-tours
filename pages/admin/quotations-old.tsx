...
export default function QuotationsAdmin() {
  ...
  const [oneTimeTours, setOneTimeTours] = useState<any[]>([]);
  const [showEditTourModal, setShowEditTourModal] = useState(false);
  const [editingTour, setEditingTour] = useState<any>(null);
  const [editingTourFromQuotation, setEditingTourFromQuotation] = useState<Quotation | null>(null);
  ...
  const fetchOneTimeTours = async () => {
    try {
      const res = await fetch('/api/one-time-tours');
      if (res.ok) {
        const data = await res.json();
        setOneTimeTours(Array.isArray(data) ? data : []);
      }
    } catch (error) {
  ...
  const handleSaveEditedTour = async (updatedTourData: any) => {
    if (!updatedTourData) return;

    try {
      // Check if it's a one-time tour (starts with OT) or a package tour (starts with PKG)
      const tourId = updatedTourData.tour_id || updatedTourData.package_id;
      
      if (!tourId) {
        logger.error('TOUR_EDIT', 'Missing tour ID in updated data', 'No tour_id or package_id provided', {
      ...
      const isOneTimeTour = tourId.startsWith('OT');
      const endpoint = isOneTimeTour 
        ? `/api/tours/${tourId}` 
        : `/api/packages/${tourId}`;

      logger.log('TOUR_EDIT', 'Starting tour update', {
        tourId,
        isOneTimeTour,
        endpoint,
        tourName: updatedTourData.tour_name || updatedTourData.package_name
      });

      // Prepare payload based on tour type
      const payload = isOneTimeTour ? {
        tourName: updatedTourData.tour_name || updatedTourData.package_name,
        ...
        includings: updatedTourData.includings,
        itinerary: updatedTourData.itinerary,
        days: updatedTourData.days,
        nights: updatedTourData.nights
      } : {
        // For package tours, use snake_case field names
        package_name: updatedTourData.tour_name || updatedTourData.package_name,
        description: updatedTourData.description,
        price: updatedTourData.price,
        image: updatedTourData.image,
        highlights: updatedTourData.highlights,
        includings: updatedTourData.includings,
        // Package model expects itinerary as string
        itinerary: typeof updatedTourData.itinerary === 'string' 
          ? updatedTourData.itinerary 
          : updatedTourData.itinerary ? JSON.stringify(updatedTourData.itinerary) : null,
        days: updatedTourData.days
      };

      logger.debug('TOUR_EDIT', 'Sending update payload', {
        tourType: isOneTimeTour ? 'one-time' : 'package',
        fields: Object.keys(payload),
        itineraryType: typeof payload.itinerary || typeof payload.itinerary
      });
      ...
      logger.debug('TOUR_EDIT', 'API response received', {
      ...
      if (!contentType || !contentType.includes('application/json')) {
        ...
        logger.error('TOUR_EDIT', 'Non-JSON response from API', `Status ${res.status}`, {
        ...
        if (res.status === 401) {
          logger.warn('TOUR_EDIT', 'Authentication required', 'Got 401 Unauthorized');
          alert('Authentication required. Please log in again.');
          window.location.href = '/admin/login';
          return;
        }
        
        alert(`Server error (${res.status}): Received non-JSON response. Check console for details.`);
        return;
      }

      const data = await res.json();
      
      logger.debug('TOUR_EDIT', 'Parsed API response', {
        success: data.success,
        hasError: !!data.error
      });
      
      if (data.success) {
        logger.log('TOUR_EDIT', 'Tour updated successfully', {
          tourId,
          isOneTimeTour
        });
        ...
        // If we were editing from quotation context, reopen the quotation modal
        if (editingTourFromQuotation) {
          logger.debug('TOUR_EDIT', 'Reopening quotation modal', {
            quotationNumber: editingTourFromQuotation.quotation_number
          });
          setEditingQuotation(editingTourFromQuotation);
          setShowEditModal(true);
          setEditingTourFromQuotation(null);
        }
        
        if (isOneTimeTour) {
          logger.debug('TOUR_EDIT', 'Refreshing one-time tours list');
          fetchOneTimeTours();
        } else {
          logger.debug('TOUR_EDIT', 'Refreshing service data');
          loadServiceData();
        }
      } else {
        logger.error('TOUR_EDIT', 'API returned error', data.error || 'Unknown error', {
          tourId,
          statusCode: res.status
        });
      ...
      logger.error('TOUR_EDIT', 'Exception updating tour', error instanceof Error ? error : new Error(String(error)), {
        tourName: updatedTourData?.tour_name || updatedTourData?.package_name
      });
  ...
  const handleDeleteTour = async (tourId: string) => {
    if (!confirm('Are you sure you want to delete this one-time tour?')) return;
  ...
  const handleCreateQuotation = async (e: React.FormEvent) => {
    e.preventDefault();
    logger.log('QUOTATION_FORM', 'Form submission started', {
      serviceType: formData.serviceType,
      customerEmail: formData.customerEmail,
      tourSaveType,
      hasOneTimeTourData: !!newTourData.tourName
    });
    ...
    if (!formData.tourName?.trim() && formData.serviceType === 'tour') errors.push('Tour name is required');
    ...
    if (formData.startDate && formData.endDate) {
      ...
      if (end.getTime() === start.getTime() && formData.durationDays < 1) {
    ...
    if (formData.numChildren < 0 || formData.numInfants < 0) {
      errors.push('Number of children/infants cannot be negative');
    }
    
    // Service selection validation 
    if (formData.serviceType === 'tour' && !formData.serviceId && tourSaveType !== 'one-time') {
      errors.push('Please select a tour package or create a one-time tour');
    }
    
    if (errors.length > 0) {
      logger.warn('QUOTATION_FORM', 'Validation failed', { errorCount: errors.length, errors });
      alert('Please fix the following errors:\n\n' + errors.join('\n'));
      return;
    }
    
    logger.debug('QUOTATION_FORM', 'Validation passed', {
      serviceType: formData.serviceType,
    ...
    try {
      ...
      if (formData.serviceType === 'vehicle' && vehicleImageInputRef.current?.files && vehicleImageInputRef.current.files.length > 0) {
        logger.log('IMAGE_UPLOAD', 'Vehicle image upload starting', {
          fileCount: vehicleImageInputRef.current.files.length
        });
        ...
        for (let i = 0; i < files.length; i++) {
          const file = files[i];
          logger.trace('IMAGE_UPLOAD', `Uploading image ${i + 1}/${files.length}`, {
            fileName: file.name,
            fileSize: file.size,
            fileType: file.type
          });
          ...
          try {
            ...
            if (res.ok) {
              ...
              if (data.url) {
                ...
                logger.debug('IMAGE_UPLOAD', `Image ${i + 1} uploaded successfully`, {
                  url: data.url
                });
              }
            } else {
              const errorData = await res.json();
              logger.error('IMAGE_UPLOAD', `Upload error for image ${i + 1}`, errorData.error, {
            ...
            logger.error('IMAGE_UPLOAD', `Exception uploading image ${i + 1}`, error instanceof Error ? error : new Error(String(error)), {
              fileName: file.name
            });
        ...
        if (uploadedUrls.length > 0) {
          ...
          logger.log('IMAGE_UPLOAD', 'All vehicle images uploaded', {
        ...
        logger.debug('IMAGE_UPLOAD', 'Skipping image upload', {
          serviceType: formData.serviceType,
          fileCount: vehicleImageInputRef.current?.files?.length || 0
        });
      }
      ...
      if ((formData.serviceType === 'airport-transfer' || formData.serviceType === 'all-island-transfer') && !formData.tourName && formData.pickupLocation && formData.dropoffLocation) {
        ...
        logger.debug('QUOTATION_FORM', 'Auto-generated transfer tour name', {
          tourName: finalFormData.tourName
        });
      }
      ...
      logger.log('QUOTATION_API', 'Sending quotation to API', {
        serviceType: quotationData.serviceType,
        customer: quotationData.customerEmail,
        hasImages: !!finalVehicleImageUrls,
        imageCount: finalVehicleImageUrls?.length || 0
      });
      ...
      const data = await res.json();
      logger.debug('QUOTATION_API', 'API response received', {
        statusCode: res.status,
        success: data.success,
      ...
      if (data.success) {
        ...
        logger.log('QUOTATION_API', 'Quotation created successfully', {
          quotationNumber,
          quotationId
        });
        
        // If we have one-time tour data, create it now
        if (tourSaveType === 'one-time' && newTourData.tourName) {
          try {
            logger.log('ONE_TIME_TOUR', 'Creating one-time tour', {
              quotationId,
              tourName: newTourData.tourName,
              days: newTourData.days
            });
            
            const tourRes = await fetch('/api/tours/create', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                tourName: newTourData.tourName,
            ...
            if (!tourData.success) {
              logger.error('ONE_TIME_TOUR', 'Failed to create one-time tour', tourData.error || 'Unknown error', {
                quotationId,
                tourName: newTourData.tourName
              });
              alert('⚠️ Quotation created, but failed to create tour package: ' + tourData.error);
            } else {
              logger.log('ONE_TIME_TOUR', 'One-time tour created successfully', {
                tourId: tourData.tour?.tourId,
                quotationId
              });
            }
          } catch (tourError) {
            logger.error('ONE_TIME_TOUR', 'Exception creating one-time tour', tourError instanceof Error ? tourError : new Error(String(tourError)), {
              quotationId
            });
            alert('⚠️ Quotation created, but failed to create tour package');
          }
        }
        ...
        logger.log('QUOTATION_FORM', 'Resetting form state', {
          quotationNumber
        });
        ...
        logger.error('QUOTATION_API', 'API returned error response', errorMsg, {
          details: data.details
        });
        alert('❌ Error creating quotation: ' + errorMsg + errorDetails);
      }
    } catch (error) {
      logger.error('QUOTATION_FORM', 'Exception in quotation creation', error instanceof Error ? error : new Error(String(error)));
  ...
  const handleCreateNewTour = async () => {
    ...
    // For one-time tours without a quotation ID, just save the config
    // The tour will be created after the quotation is saved
    if (tourSaveType === 'one-time') {
      alert('✅ Tour package configured! It will be created and linked when you save the quotation.');
      setShowCreateTourModal(false);
      return;
    }

    // For package tours, create immediately
    ...
    try {
      const res = await fetch('/api/tours/create', {
        ...
        body: JSON.stringify({
          ...
          quotationId: tourSaveType === 'one-time' ? currentQuotationId : null,
          createdBy: 'admin'
        })
      });
  ...
  const filterQuotationsByDateRange = (quotations: Quotation[]) => {
    if (!dateRangeStart && !dateRangeEnd) return quotations;
  ...
  const handleEditQuotationPackage = async (quotation: Quotation) => {
    const packageId = quotation.package_id || quotation.service_id;
    const serviceType = quotation.service_type || 'tour';

    logger.log('PACKAGE_EDIT', 'Edit package initiated', {
    ...
    if (serviceType !== 'tour') {
      logger.warn('PACKAGE_EDIT', 'Non-tour service type', { serviceType });
      alert('This quotation is not for a tour package.');
      return;
    }

    if (!packageId) {
      logger.warn('PACKAGE_EDIT', 'No package ID found', { quotationNumber: quotation.quotation_number });
      alert('No package ID found for this quotation.');
      return;
    }

    try {
      const isOneTimeTour = packageId.startsWith('OT');
      const endpoint = isOneTimeTour ? `/api/tours/${packageId}` : `/api/packages/${packageId}`;

      logger.debug('PACKAGE_EDIT', 'Fetching package details', {
        endpoint,
        isOneTimeTour,
        packageId
      });
      ...
      const res = await fetch(endpoint, {
        credentials: 'include' // Include cookies for potential auth requirements
      });
      const contentType = res.headers.get('content-type');

      if (!contentType || !contentType.includes('application/json')) {
        const textResponse = await res.text();
        logger.error('PACKAGE_EDIT', 'Non-JSON response received', `Status ${res.status}`, {
          responsePreview: textResponse.substring(0, 200)
        });
      ...
      if (!res.ok) {
        logger.error('PACKAGE_EDIT', 'API error response', data.error || 'Failed to fetch', {
          statusCode: res.status
        });
        alert(data.error || 'Failed to fetch package details');
        return;
      }

      let tourData: any = data;

      if (isOneTimeTour) {
        const source = data.tour || data;
        tourData = {
          tour_id: source.tourId || source.tour_id || packageId,
          tour_name: source.tourName || source.tour_name || quotation.tour_name,
          description: source.description || '',
          price: source.price || '',
          image: source.image || '',
          highlights: source.highlights || '',
          includings: source.includings || '',
          itinerary: source.itinerary || null,
          days: source.days || quotation.duration_days || 1,
          nights: source.nights || Math.max(0, (source.days || quotation.duration_days || 1) - 1)
        };
        logger.debug('PACKAGE_EDIT', 'Mapped one-time tour data', {
        ...
        tourData = {
          ...
          package_id: data.package_id || data.packageId || packageId,
          package_name: data.package_name || data.packageName || quotation.tour_name,
          tour_name: data.package_name || data.packageName || quotation.tour_name,
          description: data.description || '',
          price: data.price || '',
          image: data.image || '',
          highlights: data.highlights || '',
          includings: data.includings || '',
          itinerary,
          days: data.days || quotation.duration_days || 1,
          nights: data.nights || Math.max(0, (data.days || quotation.duration_days || 1) - 1)
        };
        logger.debug('PACKAGE_EDIT', 'Mapped package tour data', {
          packageId: tourData.package_id,
          packageName: tourData.package_name,
          days: tourData.days
        });
      }

      logger.log('PACKAGE_EDIT', 'Opening edit modal', {
        tourType: isOneTimeTour ? 'one-time' : 'package',
        tourId: tourData.tour_id
      });

      setEditingTour(tourData);
      setEditingTourFromQuotation(quotation);
      setShowEditTourModal(true);
      setShowEditModal(false);
    } catch (error) {
      logger.error('PACKAGE_EDIT', 'Exception fetching package details', error instanceof Error ? error : new Error(String(error)), {
        quotationNumber: quotation.quotation_number
      });
  ...
  const handleSaveEditedQuotation = async (updatedQuotation: Quotation) => {
    if (!updatedQuotation) return;

    try {
      const res = await fetch(`/api/quotations/${updatedQuotation.quotation_id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          tour_name: updatedQuotation.tour_name,
          customer_name: updatedQuotation.customer_name,
          customer_email: updatedQuotation.customer_email,
          customer_phone: updatedQuotation.customer_phone,
          start_date: updatedQuotation.start_date,
          end_date: updatedQuotation.end_date,
          num_adults: updatedQuotation.num_adults,
          num_children: updatedQuotation.num_children,
      ...
      if (data.success) {
        ...
        alert('Error updating quotation: ' + (data.error || 'Unknown error'));
      }
    } catch (error) {
    ...
    <>
      ...
      <div className="flex min-h-screen bg-gray-100">
        ...
        <div className="flex-1 admin-content-wrapper" style={{ overflowX: 'auto' }}>
          ...
          <div className="bg-white p-4 rounded-lg shadow mb-6 flex flex-wrap gap-4 items-center justify-between">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <input
                ...
                className="border border-gray-300 rounded-lg px-4 py-2"
                title="Filter quotations created from this date"
              />
              ...
              <input
                ...
                className="border border-gray-300 rounded-lg px-4 py-2"
                title="Filter quotations created until this date"
              />
              ...
              {(dateRangeStart || dateRangeEnd) && (
                <button
                  ...
                  className="border border-gray-300 rounded-lg px-4 py-2 bg-gray-100 hover:bg-gray-200 text-sm"
                >
                  ✕ Clear Dates
                </button>
              )}
                    ...
                    {filterQuotationsByDateRange(
                      quotations.filter(q => {
                        ...
                        const matchesSearch = searchTerm === '' || 
                          ...
                          q.tour_name.toLowerCase().includes(searchTerm.toLowerCase());
                            ...
                            {quotation.status === 'draft' && (
                              <>
                                <button
                                  ...
                                  className="inline-flex items-center px-3 py-1.5 bg-amber-50 text-amber-700 border border-amber-200 rounded-md hover:bg-amber-100 text-xs font-medium"
          ...
          {/* One-Time Tours Section */}
          <div className="mt-8">
            <h2 className="text-2xl font-bold mb-4">One-Time Tours</h2>
            <div className="bg-white rounded-lg shadow overflow-hidden">
              {oneTimeTours.length === 0 ? (
                <div className="p-12 text-center text-gray-500">
                  No one-time tours created yet.
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-gray-50 border-b">
                      <tr>
                        ...
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Days/Nights</th>
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {oneTimeTours.map((tour) => (
                        <tr key={tour.tour_id} className="hover:bg-gray-50">
                          <td className="px-6 py-4 whitespace-nowrap font-semibold text-blue-600">{tour.tour_id}</td>
                          <td className="px-6 py-4 max-w-xs truncate">{tour.tour_name}</td>
                          <td className="px-6 py-4 whitespace-nowrap font-bold">${tour.price?.toFixed(2) || '0.00'}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm">
                            {tour.days} Days {tour.nights ? `/ ${tour.nights} Nights` : ''}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          <div className="flex gap-2">
                            ...
                            {formData.serviceId && (
                              <button
                                type="button"
                                onClick={() => {
                                  const tour = tours.find(t => t.package_id === formData.serviceId);
                                  if (tour) {
                                    ...
                                    setEditingTour({
                                      tour_id: tour.package_id,
                                      package_id: tour.package_id,
                                      tour_name: tour.package_name,
                                ...
                                className="px-4 py-2 bg-amber-600 text-white rounded hover:bg-amber-700 font-medium transition"
                                title="Edit selected tour package"
                              >
                                ✏️ Edit
                              </button>
                            )}
      ...
      {showEditModal && editingQuotation && (
        <EditQuotationModal
          ...
          onEditPackage={handleEditQuotationPackage}
        />
      )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="bg-white rounded-lg max-w-4xl w-full p-6 my-8 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleCreateNewTour(); }} className="space-y-5">
              ...
              {/* One-Time Tour Info */}
              {tourSaveType === 'one-time' && (
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                  <p className="text-sm font-semibold text-amber-900 mb-1">
                    📋 One-Time Tour Package
                  </p>
                  <p className="text-xs text-amber-700">
                    This tour will be automatically linked to the quotation you're creating
                  </p>
                </div>
              )}
...
function EditTourModal({ tour, onClose, onSave }: { tour: any; onClose: () => void; onSave: (data: any) => void }) {
  ...
  const handleDaysChange = (newDays: number) => {
    setEditData({ ...editData, days: newDays, nights: Math.max(0, newDays - 1) });
    
    const currentLength = dayItineraries.length;
    if (newDays > currentLength) {
      const newItems = Array.from({ length: newDays - currentLength }, (_, i) => ({
        day: currentLength + i + 1,
        title: '',
        description: '',
        activities: '',
      ...
      setDayItineraries(dayItineraries.slice(0, newDays));
    }
  };
  ...
  const handleSaveWithItinerary = () => {
    // Update editData with itinerary before saving
    const updatedData = { ...editData, itinerary: dayItineraries };
    // Call parent save with updated data
    onSave(updatedData);
  };

  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
      <div className="bg-white rounded-lg shadow-lg max-w-4xl w-full my-8 max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white flex justify-between items-center px-6 py-4 border-b z-10">
          <h2 className="text-xl font-bold">Edit Tour: {tour.tour_id}</h2>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <input
                type="text"
                value={editData.tour_name || editData.package_name || ''}
                onChange={(e) => setEditData({ ...editData, tour_name: e.target.value, package_name: e.target.value })}
            ...
            <div>
              ...
              <input
                type="number"
                min="0"
                value={editData.nights || 0}
                onChange={(e) => setEditData({ ...editData, nights: parseInt(e.target.value) || 0 })}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-emerald-500"
              />
            </div>
            <div className="col-span-2">
              ...
              <textarea
                ...
                className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-emerald-500"
                rows={2}
                placeholder="Sigiriya Rock Fortress, Kandy Temple, Elephant Safari, etc."
              />
            </div>
            <div className="col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-1">Includings (comma-separated)</label>
              <textarea
                value={editData.includings || ''}
                onChange={(e) => setEditData({ ...editData, includings: e.target.value })}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-emerald-500"
                rows={2}
                placeholder="Accommodation, Meals, Transportation, Guide, etc."
              />
            </div>
          </div>
            ...
            <div className="space-y-4">
              {dayItineraries.map((day, index) => (
                <div key={index} className="border rounded-lg p-4 bg-gray-50">
                  <h4 className="font-semibold text-blue-700 mb-3">Day {day.day}</h4>
        ...
        <div className="sticky bottom-0 bg-white flex gap-3 px-6 py-4 border-t">
          <button
            onClick={onClose}
          ...
          <button
            onClick={handleSaveWithItinerary}
...
function EditQuotationModal({ 
  ...
  onEditPackage: (quotation: Quotation) => void;
}) {
  const [editData, setEditData] = useState<Quotation>(quotation);
  const hasPackage = (editData.service_type || 'tour') === 'tour' && (editData.package_id || editData.service_id);

  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-lg max-w-2xl w-full max-h-screen overflow-y-auto">
        <div className="flex justify-between items-center px-6 py-4 border-b">
          <h2 className="text-xl font-bold">Edit Quotation: {quotation.quotation_number}</h2>
        ...
        <div className="p-6 space-y-4">
          {hasPackage && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-blue-900">Associated Tour Package</p>
                  <p className="text-xs text-blue-700 mt-1">Package ID: {editData.package_id || editData.service_id}</p>
                </div>
                <button
                  onClick={() => onEditPackage(editData)}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                type="text"
                value={editData.tour_name}
                onChange={(e) => setEditData({ ...editData, tour_name: e.target.value })}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-emerald-500"
              />
            </div>
            <div>
            ...
            <div>
              ...
              <input
                type="number"
                min="1"
                value={editData.num_adults}
                onChange={(e) => setEditData({ ...editData, num_adults: parseInt(e.target.value) || 1 })}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-emerald-500"
              />
            </div>
            <div>
              ...
              <input
                type="number"
                min="0"
                value={editData.num_children}
                onChange={(e) => setEditData({ ...editData, num_children: parseInt(e.target.value) || 0 })}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-emerald-500"
              />
            </div>
            <div className="col-span-2">
